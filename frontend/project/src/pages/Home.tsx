...
const ResumeUploadSection = ({ onAnalysisComplete }: { onAnalysisComplete: (analysis: ResumeAnalysis) => void }) => {
  ...
  return (
    <div className="max-w-xl mx-auto">
      <div className="border-2 border-dashed border-stone-800 rounded-lg p-10 text-center hover:border-amber-400/40 transition-all">
        <input
          type="file"
          id="resume"
          className="hidden"
        ...
        <label htmlFor="resume" className="cursor-pointer block">
          ...
          <p className="text-stone-500 text-sm">Supported format: PDF</p>
        </label>
        ...
        <div className="mt-6 max-w-md mx-auto">
          ...
          <select
            ...
            disabled={isProcessing}
            className="w-full bg-stone-900 border border-stone-800 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-amber-400/30 focus:border-transparent disabled:opacity-50"
          >
        ...
        {resumeFile && !isProcessing && (
          <div className="mt-6">
            ...
            <button
              ...
              disabled={isProcessing}
              className="bg-amber-400 hover:bg-amber-300 text-stone-900 px-6 py-2 rounded-lg transition-colors flex items-center space-x-2 mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {latestResume && (
          <div className="mt-6 p-4 bg-stone-900/60 rounded-lg text-left">
            <h4 className="font-medium text-amber-400 mb-2">Latest Resume</h4>
            <p className="text-stone-300">{latestResume.fileName}</p>
  ...
  if (!analysis) {
    return (
      <div className="text-center py-16">
        <p className="text-stone-400">Upload and analyze your resume to see job recommendations</p>
      </div>
    );
  }
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="flex items-center space-x-2 px-4 py-2 bg-amber-400/10 hover:bg-stone-800 rounded-lg transition-colors"
        >
      ...
      {showFilters && (
        <div className="mb-6 p-4 bg-stone-900/60 rounded-lg border border-stone-800">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm text-stone-400 mb-2">Location</label>
              <input
                type="text"
                value={filters.location}
                onChange={(e) => handleFilterChange('location', e.target.value)}
                placeholder="e.g., Remote, New York"
                className="w-full px-4 py-2 bg-stone-900 rounded-lg border border-stone-800 focus:border-amber-400/40 outline-none"
              />
            </div>
            <div>
              <label className="block text-sm text-stone-400 mb-2">Keywords</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm text-stone-400 mb-2">Posted within (days)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm text-stone-400 mb-2">Min Match Score</label>
              <input
                type="number"
          ...
          <button
            ...
            className="w-full bg-amber-400 hover:bg-amber-300 text-stone-900 py-2 rounded-lg transition-colors"
          >
            Apply Filters
          </button>
        </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-16">
          <Loader2 className="w-8 h-8 text-amber-400 animate-spin" />
          <span className="ml-3 text-stone-400">Finding best job matches...</span>
        </div>
      ) : error ? (
        <div className="text-center py-16">
          ...
          <button
            ...
            className="mt-4 px-6 py-2 bg-amber-400 hover:bg-amber-300 rounded-lg transition-colors"
          >
            Retry
          </button>
        </div>
        ...
        <div className="text-center py-16">
          <p className="text-stone-400">No job recommendations found. Try adjusting your filters.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {jobs.map((job, index) => (
            <div
              key={index}
              className="bg-stone-900 rounded-lg p-6 border border-stone-800 hover:border-amber-400/30 transition-all"
            >
              ...
              <div className="flex flex-wrap gap-4 mb-3 text-sm text-stone-400">
              ...
              {/* Job Description */}
              <div className="mb-4 p-4 bg-stone-950 rounded-lg border border-stone-800">
                <p className="text-sm font-medium text-amber-300 mb-2">Job Description:</p>
                <p 
                  className="text-stone-300 text-sm whitespace-pre-line"
                  dangerouslySetInnerHTML={{ 
                    __html: job.snippet
                      .replace(/&nbsp;/g, ' ')
                      .replace(/<b>/g, '<strong class="text-amber-300">')
                      .replace(/<\/b>/g, '</strong>')
                      .replace(/<br>/g, '\n')
                      .replace(/<br\/>/g, '\n')
                  }}
              ...
              {job.recommendationReasons && job.recommendationReasons.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm font-medium text-amber-300 mb-2">Match Analysis:</p>
                  <ul className="space-y-1">
                    {job.recommendationReasons.map((reason, i) => (
                      <li key={i} className="text-sm text-stone-400 flex items-start">
              ...
              <a
                ...
                className="inline-flex items-center px-4 py-2 bg-amber-400 hover:bg-amber-300 text-stone-900 rounded-lg transition-colors"
              >
                View Job
  ...
  return (
    <ResumeProvider>
      <div className="min-h-screen bg-[#0C0A09] text-stone-100">
        <nav className="fixed w-full z-50 bg-[#0C0A09] shadow-lg border-b border-stone-800">
          <div className="container mx-auto px-4 py-4 flex justify-between items-center">
            <span className="text-xl font-bold">
              Resume<span className="text-amber-400">Score</span>
            </span>
            <button
              onClick={handleLogout}
              className="flex items-center text-stone-300 hover:text-amber-300 transition-colors"
            >
        ...
        <main className="container mx-auto px-4 pt-24 pb-16">
          <section className="py-8">
            <div className="max-w-4xl mx-auto text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-bold mb-6">
                <span className="text-amber-400">Optimize</span> Your Resume for ATS
              </h1>
          ...
          {analysis && (
            <>
              {/* ATS Score Section */}
              <section className="py-8 bg-stone-900/60 rounded-lg mb-8 border border-stone-800">
                  ...
                  <div className="text-center p-8">
                    <div className="w-32 h-32 mx-auto mb-4 relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-3xl font-bold text-amber-400">{analysis.score}</span>
                      </div>
                    </div>
                    <h2 className="text-2xl font-bold mb-2">ATS Score</h2>
                    <p className="text-stone-400">{analysis.statusMessage}</p>
                  </div>
                  <div className="p-8">
                    ...
                    <ul className="space-y-3">
                      {analysis.insights.slice(0, 5).map((insight, i) => (
                        <li key={i} className="flex items-start">
                          <span className="w-6 h-6 rounded-full bg-amber-400/20 text-amber-400 flex items-center justify-center mr-3 mt-0.5 flex-shrink-0">
                            ✓
                          </span>
                          <span className="text-stone-300">{insight}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="bg-green-950/20 rounded-lg p-6 border border-green-900/30">
                    ...
                    <ul className="space-y-3">
                      {analysis.insights
                        ...
                        .map((quality, i) => (
                          <li key={i} className="flex items-start text-sm">
                            ...
                            <span className="text-stone-300">{quality}</span>
                          </li>
                        ))}
                      {analysis.insights.filter(insight => 
                        ...
                        <li className="text-sm text-stone-400 italic">Analyzing resume strengths...</li>
                      )}
                    </ul>
                  </div>
                    ...
                    <ul className="space-y-3">
                      {analysis.recommendations.slice(0, 6).map((drawback, i) => (
                        <li key={i} className="flex items-start text-sm">
                          ...
                          <span className="text-stone-300">{drawback}</span>
                        </li>
                      ))}
                      {analysis.recommendations.length === 0 && (
                        <li className="text-sm text-stone-400 italic">No major issues found!</li>
                      )}
                    </ul>
                  </div>
                </div>
